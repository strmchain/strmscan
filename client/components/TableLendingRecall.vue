<template>
    ...
    <section v-else>
        <p
            v-if="total > 0"
            class="strm-total-items">{{ _nFormatNumber('lending recall', 'lending recalls', total) }}</p>
        <form
            v-if="showFilter"
        ...
        <div
            ...
            <p class="strm-empty__description">No recall found</p>
        </div>
        <table-base
            v-if="total > 0"
            :fields="fields"
            :items="items"
            class="strm-table--lending-recalls">
            <template
                slot="hash"
                slot-scope="props">
                <i
...
export default {
    ...
    data: () => ({
        ...
        collateralToken: '',
        status: '',
        strmNativeToken: process.env.STRM_NATIVE_TOKEN
    }),
    async created () {
    ...
    methods: {
        async getDataFromApi () {
            ...
            const { data } = await this.$axios.get('/api/lending/recalls?' + query)
