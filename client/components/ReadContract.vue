<template>
    ...
    <div
        v-else
        class="card strm-card strm-card--contract-info">

        <div
            v-if="data.length == 0"
            class="strm-empty">
            <i class="fa fa-book strm-empty__icon"/>
            <p class="strm-empty__description">Can not read this smart contract</p>
        </div>

        <div class="strm-card__header mb-3">
        ...
        <table class="strm-card__table strm-contract-info">
            <tr
                v-for="(func, index) in data"
                :key="index">
                <td :class="`strm-contract-info__func ${func.inputs.length ? 'with-input' : 'no-input'}`">
                    <span class="strm-contract-info__name">
                    ...
                    <span
                        v-if="typeof func.result !== 'undefined' && ! func.inputs.length"
                        class="strm-contract-info__result">
                        <nuxt-link
                        ...
                        <em class="strm-contract-info__type">{{ func.outputs[0].type }}</em>
                    </span>
                    <div
                        v-if="func.inputs.length"
                        class="strm-contract-info__inputs">
                        <input
                    ...
                    <div
                        ...
                        class="strm-contract-info__outputs">
                        <i class="fa fa-angle-double-right"/>
                        <span
                            v-for="(output, idx2) in func.outputs"
                            :key="idx2">
                            <span class="strm-contract-info__param-name">{{ output.name }}</span>
                            <em class="strm-contract-info__type">{{ output.type }}</em>
    ...
    methods: {
        ...
        async callFunction (functionName, signature, inputElement, outputElement) {
            ...
            document.getElementById(outputElement).innerHTML =
                `<div class="strm-contract-info__response">
        ...
        formatOuputs (output) {
            ...
            for (let i = 0; i < output.length; i++) {
                ...
                response += `<strong>${output[i].name}</strong>&nbsp;
                    <em class="strm-contract-info__type">${output[i].type}</em> :
        ...
        resetInput (e) {
            e.preventDefault()
            const inputs = document.querySelectorAll('.strm-contract-info input')
            const ouputs = document.querySelectorAll('.strm-contract-info__func.with-input .strm-contract-info__result')
