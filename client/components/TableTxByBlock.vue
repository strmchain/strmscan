<template>
    ...
    <section v-else>
        ...
        <div
            v-if="total == 0"
            class="strm-empty">
            <i class="fa fa-exchange strm-empty__icon"/>
            <p class="strm-empty__description">No transaction found</p>
        </div>

        <p
            v-if="total > 0"
            class="strm-total-items">{{ _nFormatNumber('transaction', 'transactions', total) }}</p>

        <table-base
            v-if="total > 0"
            :fields="fields"
            :items="items"
            class="strm-table--transactions">
            <template
                slot="hash"
                slot-scope="props">
                <nuxt-link
            ...
            <template
                slot="value"
                slot-scope="props">{{ formatUnit(toStrm(props.item.value)) }}</template>
            ...
            <template
                ...
                <span
                    v-b-tooltip.hover
                    :title="formatUnit(toStrm(props.item.gasPrice * props.item.gasUsed, 18))">
                    {{ formatUnit(toStrm(props.item.gasPrice * props.item.gasUsed, 8)) }}</span></template>
        </table-base>
        ...
        async getDataFromApi () {
            ...
            data.items.forEach(async (item, index, array) => {
                if (index === array.length - 1) {
                    ...
                    // Format data.
                    if (self.blockNumber) {
                        self.items = self.formatData(self.items, self.blockTimestamp)
                    } else {
