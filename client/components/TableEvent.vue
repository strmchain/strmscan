<template>
    ...
    <section v-else>
        ...
        <div
            v-if="total == 0"
            class="strm-empty">
            <i class="fa fa-binoculars strm-empty__icon"/>
            <p class="strm-empty__description">No event found</p>
        </div>

        <p
            v-if="total > 0"
            class="strm-total-items">{{ _nFormatNumber('item', 'items', total) }}</p>

        <table-base
            v-if="total > 0"
            :fields="fields"
            :items="items"
            class="strm-table--events">
