<template>
    ...
    <section v-else>
        ...
        <div
            ...
            <p class="strm-empty__description">No reward found</p>
        </div>

        <p
            v-if="total > 0"
            class="strm-total-items">{{ _nFormatNumber('reward', 'rewards', total) }}</p>

        <table-base
            v-if="total > 0"
            :fields="fields"
            :items="items"
            class="strm-table--reward strm-table--reward-by-epoch">
            <template
                slot="address"
                slot-scope="props">
                <nuxt-link
