<template>
    ...
    <section v-else>
        <p
            v-if="total > 0"
            class="strm-total-items">{{ _nFormatNumber('lending repay', 'lending repays', total) }}</p>
        <form
            v-if="showFilter"
        ...
        <div
            ...
            <p class="strm-empty__description">No repay found</p>
        </div>
        <table-base
            v-if="total > 0"
            :fields="fields"
            :items="items"
            class="strm-table--lending-repay">
            <template
                slot="hash"
                slot-scope="props">
                <nuxt-link
