<template>
    ...
    <section v-else>
        ...
        <div
            ...
            <p class="strm-empty__description">No holder found</p>
        </div>

        <p
            v-if="total > 0"
            class="strm-total-items">{{ _nFormatNumber('holder', 'holders', total) }}</p>

        <table-base
            v-if="total > 0"
            :fields="fields"
            :items="items"
            class="strm-table--holders">
            <template
                slot="hash"
                slot-scope="props">
                <nuxt-link
...
export default {
    ...
    methods: {
        async getDataFromApi () {
            ...
            const { data } = await this.$axios.get('/api/token-holders/trc21' + '?' + query)
