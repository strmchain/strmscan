<template>
    ...
    <section v-else>
        <div
            v-if="total == 0"
            class="strm-empty">
            <i class="fa fa-cubes strm-empty__icon"/>
            <p class="strm-empty__description">No Epoch found</p>
        </div>

        <p
            v-if="total > 0"
            class="strm-total-items">{{ _nFormatNumber('epoch', 'epochs', total) }}</p>

        <table-base
            v-if="total > 0"
            :fields="fields"
            :items="items"
            class="strm-table--epochs">
