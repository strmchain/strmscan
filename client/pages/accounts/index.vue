<template>
    ...
    <section v-else>
        <div
            ...
            <p class="strm-empty__description">No account found</p>
        </div>

        <p
            v-if="total > 0"
            class="strm-total-items">{{ _nFormatNumber('account', 'accounts', total) }}</p>

        <table-base
            v-if="total > 0"
            :fields="fields"
            :items="items"
            class="strm-table--accounts">
            ...
            <template
                slot="balance"
                slot-scope="props">
                <span class="d-lg-none">{{ formatUnit(toStrm(props.item.balance, 5)) }}</span>
                <span class="d-none d-lg-block">{{ formatUnit(toStrm(props.item.balance)) }}</span>
            </template>
        </table-base>
