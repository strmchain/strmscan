<template>
    ...
    <section v-else>
        <div
            ...
            <p class="strm-empty__description">No relayer found</p>
        </div>

        <p
            v-if="total > 0"
            class="strm-total-items">{{ _nFormatNumber('relayer', 'relayers', total) }}</p>

        <table-base
            v-if="total > 0"
            :fields="fields"
            :items="items"
            class="strm-table--relayers">
            <template
                slot="address"
                slot-scope="props">
                <nuxt-link
            ...
            <template
                slot="deposit"
                slot-scope="props">
                <span class="d-lg-none">{{ formatUnit(toStrm(props.item.deposit, 5)) }}</span>
                <span class="d-none d-lg-block">{{ formatUnit(toStrm(props.item.deposit)) }}</span>
            </template>
            <template
                slot="volume24h"
