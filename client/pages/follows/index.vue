<template>
    ...
    <section
        v-else
        class="strm-body-wrapper strm-body-wrapper--follow">
        <div class="mb-4">
            <b-btn
            ...
            <b-modal
                id="modalAddFollow"
                ref="modalNewAddress"
                class="strm-modal strm-modal--follow"
                ...
                <p>You can monitor and receive an alert when an address
                    on your follow list receives an incoming STRM Transaction.</p>
                <div class="form-group">
                    <div class="strm-toggle">
                        <input
                            id="emailNotify"
                            ...
                            name="sendEmail"
                            type="checkbox"
                            class="strm-toggle__checkbox">
                        <label
                            for="emailNotify"
                            class="strm-toggle__btn"/>
                        <span class="strm-toggle__label"><i class="fa fa-envelope-o mr-2"/>Email Notification</span>
                    </div>
                    <div
                        v-if="formSendEmail"
                        class="strm-toggle">
        ...
        <div
            ...
            <p class="strm-empty__description">No address found</p>
        </div>

        <p
            v-if="total > 0"
            class="strm-total-items">Total {{ formatNumber(total) }} items found</p>

        <table-base
            v-if="total > 0"
            :fields="fields"
            :items="items"
            class="strm-table--follow">
            ...
            <template
                slot="balance"
                slot-scope="props">
                <span class="d-lg-none">{{ formatUnit(toStrm(props.item.addressObj.balance, 5)) }}</span>
                <span class="d-none d-lg-block">{{ formatUnit(toStrm(props.item.addressObj.balance)) }}</span>
            </template>
