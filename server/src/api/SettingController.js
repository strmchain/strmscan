...
SettingController.get('/setting/usd', async (req, res) => {
    try {
        const cache = await redisHelper.get('strm-price')
        if (cache !== null) {
            const r = JSON.parse(cache)
            logger.info('load strm price from cache')
            return res.json(r)
        }
        const url = 'https://api.coingecko.com/api/v3/simple/price?ids=strmchain&vs_currencies=usd'

        const { data } = await axios.get(url, { timeout: 5000 })

        strmUsd = data
        await redisHelper.set('strm-price', JSON.stringify(data), 10 * 60)
    } catch (e) {
        logger.warn(e)
    }
    return res.json(strmUsd)
})
