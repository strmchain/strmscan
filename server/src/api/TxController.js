...
const StrmIssuer = require('../contracts/abi/StrmIssuer')
        ...
        if (typeof address !== 'undefined') {
            ...
            */
            ...
            if (type === 'signTxs') {
                ...
                condition = {
                    to: { $nin: [contractAddress.BlockSigner, contractAddress.StrmRandomize] }
                }
            } else if (type === 'all') {
            ...
            query = {
                bool: {
                    should: [
                        ...
                        { term: { to: contractAddress.StrmRandomize } }
                    ]
                }
            }
        } else if (type === 'normalTxs') {
    ...
    try {
        ...
        if (trc21Txs.length > 0) {
            try {
                ...
                const contract = new web3.eth.Contract(StrmIssuer, config.get('STRMISSUER'))
                const listToken = await contract.methods.tokens().call()
                let isRegisterOnStrmIssuer = false
                for (let i = 0; i < listToken.length; i++) {
                    if (listToken[i].toLowerCase() === tx.to.toLowerCase()) {
                        isRegisterOnStrmIssuer = true
                        break
                    }
                }
                if (isRegisterOnStrmIssuer) {
