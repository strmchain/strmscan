...
TokenController.get('/tokens/:slug', [
    ...
    try {
        ...
        const blacklistUrl = 'https://raw.githubusercontent.com/strmchain/tokens/master/blacklist.json'
        ...
        const verifiedUrl = 'https://raw.githubusercontent.com/strmchain/tokens/master/verifiedlist.json'
        ...
        if (!coinList.includes(hash)) {
            const bridgeUrl = 'https://raw.githubusercontent.com/strmchain/tokens/master/bridge.json'
        ...
        try {
            const moreInfoUrl = `https://raw.githubusercontent.com/strmchain/tokens/master/tokens/${hash}.json`
...
TokenController.get('/tokens/holding/:tokenType/:holder', [
    ...
    try {
        ...
        if (tokenType === 'trc20') {
            ...
            const strmxToken = await db.Token.find({ hash: { $in: listToken } })
            ...
            const notExist = []
            for (let i = 0; i < strmxToken.length; i++) {
                if (!exist.includes(strmxToken[i].hash)) {
                    notExist.push({
                        hash: holder,
                        token: strmxToken[i].hash,
                        tokenDecimals: strmxToken[i].decimals,
                        quantity: '0',
                        quantityNumber: 0
                    })
                }
