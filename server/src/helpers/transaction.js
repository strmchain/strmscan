        ...
        if (log.topics[0] === TOPIC_TRANSFER) {
            ...
            if (params.length >= 4) {
                ...
                if (srcAddress === '0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee') {
                    let strmAmount = new BigNumber(web3.utils.hexToNumberString('0x' + params[2]))
                    strmAmount = strmAmount.dividedBy(10 ** 18)
                    let constantAmount = new BigNumber(web3.utils.hexToNumberString('0x' + params[3]))
                    constantAmount = constantAmount.dividedBy(10 ** 2)

                    strmRate = constantAmount.dividedBy(strmAmount).toNumber()
                } else {
                    let strmAmount = new BigNumber(web3.utils.hexToNumberString('0x' + params[3]))
                    strmAmount = strmAmount.dividedBy(10 ** 18)
                ...
                const txExtraInfo = [
                    {
                        ...
                        infoName: 'Swap rate',
                        infoValue: `1 STRM = ${strmRate} CONST`
                    }
                ]
    ...
    crawlTransaction: async (hash, timestamp) => {
        ...
        try {
            ...
            if (tx.from !== null) {
                ...
                if (tx.to !== contractAddress.BlockSigner && tx.to !== contractAddress.StrmRandomize) {
